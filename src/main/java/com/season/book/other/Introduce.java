 ...
 * 注意：
	 ...
	 select d.dname from Dept d 
	        where exists(from Emp e where d=e.dept)
 * 
 * 	exists()
 * 
 * 本地sql
 ...
 * 
 * SQLQuery sq=session.createSQLQuery(sql);
		sq.setString("dname", "ACCOUNTING");
		sq.addEntity("e", Emp.class);
		sq.setXXX(0,"参数值")
 ...
 * 
 * 命名查询(很少用)
 * 把程序里的sql语句写在任一hbm文件(最好自己单独写)中,cfg再引用,这么着就可以实现分离了
 * 		配置文件里写一些东西
 * 1.编码文件中定义名字和语句
 * 		<query name="hql名字">
 * 			<![CDATA[hql语句]]>
 * 		</query>
 * 		<sql-query name="sql名字">
 * 			<![CDATA[sql语句]]>
 * 		</sql-query>
 * 2.dao中使用名字
 * 		Query
 * 
 * 可以使用jdbc(扩展的知识点,跟spring有关)
 * 
 * String不能传出来,final了,  但是用StringBuffer就可以
 * 常量虽然不能改地址,但是可以改数据,String不行
 * 
 * 接口,匿名内部类
 * 
 * 
